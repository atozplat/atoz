...
public class PeriodicCatchupHandler implements Runnable {

    private Blockchain blockchain;
    private HashMap<ServerInfo, Date> serverStatus;
    private int localPort;

    public PeriodicCatchupHandler(Blockchain blockchain, HashMap<ServerInfo, Date> serverStatus, int localPort) {
    ...
    public void broadcast(String message) {
        ...
        for (ServerInfo info : this.serverStatus.keySet()) {
            Thread thread = new Thread(new MessageSenderHandler(info, message));
            thread.start();
    ...
    public void multicast(ArrayList<ServerInfo> toPeers, String message) {
        ...
        for (int i = 0; i < toPeers.size(); i++) {
            Thread thread = new Thread(new MessageSenderHandler(toPeers.get(i), message));
            thread.start();
            threadArrayList.add(thread);
        }
    }
